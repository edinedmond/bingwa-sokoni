...
async function helpCommand(sock, sender) {
    ...
    const imagePath = path.join(__dirname, '../assets/welcome.png'); // Absolute path from src/commands/
    try {
        ...
        await sendMessage(sock, sender, welcomeText); // Fallback to text-only if image fails
    }
}
